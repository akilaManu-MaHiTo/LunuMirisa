...
const LeaveList = () => {
  ...
  const fetchLeaves = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/leaves/${employeeId}`);
  ...
  const handleApprove = async (leaveId) => {
    try {
      await axios.put(`http://localhost:3000/leaves/approve/${leaveId}`);
  ...
  const handleReject = async (leaveId) => {
    try {
      await axios.put(`http://localhost:3000/leaves/reject/${leaveId}`);
