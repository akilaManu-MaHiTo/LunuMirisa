...
function AcceptedOrders() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await fetch('http://localhost:3000/acceptedOrders');
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this order?")) {
      try {
        await fetch(`http://localhost:3000/acceptedOrders/${id}`, { method: 'DELETE' });
  ...
  const handleUpdateSubmit = async (e) => {
    e.preventDefault();
    try {
      await fetch(`http://localhost:3000/acceptedOrders/${updatedOrder._id}`, {
        method: 'PUT',
