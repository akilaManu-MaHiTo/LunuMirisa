...
const FoodItem = ({
  ...
  <div className="relative bg-custom-gray p-6 rounded-xl shadow-md w-[18rem] h-auto max-w-md mb-4 flex flex-col items-center transition-all duration-500 ease-in-out transform hover:scale-105 hover:shadow-[0_0_10px_rgba(255,255,255,0.6)]">
    ...
    <img
      src={image}
      alt={title}
      className="w-48 h-44 bg-cover mt-5 bg-center transition-all duration-500 hover:shadow-2xl"
    />

    <div className="text-center text-white font-spartan font-thin mt-8 text-2xl">
      {title}
    </div>

    <div className="text-center text-white font-spartan font-semibold text-xl mt-2">
      Rs.{((price * (100 - percentage)) / 100).toFixed(2)}/-{" "}
      {/* Ensure precision */}
    </div>
...
const UserHome = () => {
  ...
  useEffect(() => {
    ...
    const fetchHotDeals = async () => {
      try {
        const response = await axios.get("http://localhost:3000/getHotDeals");
  ...
  return (
    <div className="bg-custom-black min-h-screen">
      ...
      <section>
        <div className="text-white text-5xl pt-20 font-spartan font-thin pl-[10rem] select-none">
          Today's Special
        </div>
        ...
        <div className="flex justify-center gap-y-8 mb-10">
          <div className="flex flex-wrap gap-x-16 gap-y-8 pt-16 pb-10 justify-center w-[80rem]">
            {topThreeDeals.map((hot, index) => (
              <FoodItem
                key={index}
                image={`http://localhost:3000/Images/${hot.image}`}
                title={hot.title}
                price={hot.price}
          ...
          >
            ...
            <div className="absolute inset-0 flex flex-col justify-center items-start z-20 px-6 sm:px-8 md:px-12">
              ...
              <p className="text-white w-full max-w-2xl text-sm sm:text-base md:text-xl font-spartan font-light leading-relaxed select-none mb-8">
                Enjoy the finest flavors of authentic Sri Lankan cuisine at
                Lunumirisa. Whether you’re planning a special night out, a
                family gathering, or simply wish to savor an exquisite meal,
                reserving a table guarantees that you’ll be part of a culinary
                journey unlike any other. Our cozy, elegant ambiance and
                carefully crafted dishes ensure that every moment is a delight.
                Reserve your table today and let us take care of the rest—your
                unforgettable dining experience awaits!
              </p>

              <Link to={`/TableReservationPage/${userId}`}>
                <button
                  className="mt-6 w-auto h-10 sm:h-12 px-4 sm:px-5 text-black bg-white transition-all ease-in-out transform hover:scale-105 flex items-center justify-center gap-2
              ...
              >
                ...
                >
                  <div className="flex flex-col sm:flex-row items-start">
                    <img
                      src={`http://localhost:3000/Images/${reviews[currentReviewIndex].profileImage}`}
                    ...
                    <div className="flex flex-col">
                      <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-white">
                        {reviews[currentReviewIndex].FirstName}{" "}
                        {reviews[currentReviewIndex].LastName}
                      </h3>
