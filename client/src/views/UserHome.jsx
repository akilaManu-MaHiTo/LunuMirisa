...
const FoodItem = ({ image, title, price }) => (
  <div className="bg-custom-gray p-6 rounded shadow-md w-[18rem] h-auto max-w-md mb-4 md:mb-0 flex flex-col items-center transition-all duration-500 ease-in-out transform hover:scale-105">
    <img src={image} alt={title} className='w-[15rem] h-auto rounded mx-auto mt-2' />
    <div className="text-center text-white font-spartan font-thin mt-8 text-2xl">{title}</div>
    <div className="text-center text-white font-spartan font-thin text-xl mt-2">Rs.{price}/-</div>
    <button type="button" className='flex mb-4 items-center justify-center w-[15rem] py-1 mt-16 bg-custom-light text-white hover:bg-white hover:text-black h-12 transition-all duration-300 ease-in-out transform hover:scale-105'>
  ...
  useEffect(() => {
    ...
    const fetchHotDeals = async () => {
      try {
        ...
        console.error("There was an error fetching the hot deals!", error);
      }
    };
  ...
  return (
    <div className="bg-custom-black min-h-screen">
      ...
      <div className='flex justify-center gap-y-8'>
        <div className="flex flex-wrap gap-x-16 gap-y-8 pt-16 pb-10 justify-center w-[80rem]">
          {hotDeals.map((hot, index) => (
            <FoodItem key={index} image={`http://localhost:3001/Images/` + hot.image} title={hot.title} price={hot.price * 98/100} />
          ))}
        </div>
      </div>
      ...
      <div className='flex justify-center gap-y-8'>
        <div className="flex flex-wrap gap-x-16 gap-y-8 pt-16 pb-10 justify-center w-[80rem]">
          {[...Array(3)].map((_, index) => (
            <FoodItem key={index} image={food} title="Chicken Biriyani" price="1700" />
          ))}
        </div>
      </div>
      ...
      <div className="flex flex-col items-center justify-center py-8">
        {loading ? (
          ...
          >
            <div key={currentReviewIndex} className="bg-custom-dark p-10 rounded my-4 w-[45rem] h-[25rem] mx-auto">
              ...
              <div>
                ...
                <div className='bg-custom-gray h-[11rem] pl-4 pr-4 pb-4 pt-1 mt-3 rounded-xl'>
                  <p className="text-gray-300 italic mt-5">{reviews[currentReviewIndex].description}</p>
                </div>
              </div>
            </div>
          </div>
