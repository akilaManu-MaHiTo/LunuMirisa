...
const ShowWaitorOrders = () => {
  ...
  useEffect(() => {
    const fetchOrderItems = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`http://localhost:3000/OrderItems/${orderId}`);
  ...
  const handleStatusChange = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://localhost:3000/UpdateStatus/${orderId}`, { status });
  ...
  const handleUpdateQuantity = async (e, itemId) => {
    ...
    try {
      await axios.put(`http://localhost:3000/UpdateQuentity/${itemId}`, { quantity: newQuantity });
  ...
  const handleDeleteItem = async (itemId) => {
    ...
    try {
      await axios.delete(`http://localhost:3000/DeleteOrderItem/${itemId}`);
